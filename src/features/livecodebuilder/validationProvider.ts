...
const enum Setting {
	Run = 'livecodebuilder.validate.run',
	Enable = 'livecodebuilder.validate.enable',
	LivecodeServerExecutablePath = 'livecodescript.LivecodeServerExecutablePath',
	LCBCompilerPath = 'livecodebuilder.LCBCompilerPath',
}
...
let lcbconfig = vscode.workspace.getConfiguration('livecodebuilder');
let InclusionsPath = lcbconfig.get("LCBDependenciesPath", "");


export default class LivecodebuilderValidationProvider {
	

	
	private static MatchExpression: RegExp = /(?:(?:Parse|Fatal) error): (.*)(?: in )(.*?)(?: on line )(\d+)/;
	//private static BufferArgs: string[] = ['--modulepath '+InclusionsPath, '-v'];
	//private static FileArgs: string[] = ['--modulepath '+InclusionsPath,'-v'];
	
	private static BufferArgs: string[] = [path.resolve(__dirname, '../../../tools/Linter.lc').replace(/[\\]+/g,"/"), '-scope=.source.lcb', '-modulepaths='+InclusionsPath, '-lcbCompilerPath='+lcbconfig.get("LCBCompilerPath", "")];
	private static FileArgs:   string[] = [path.resolve(__dirname, '../../../tools/Linter.lc').replace(/[\\]+/g,"/"), '-scope=.source.lcb', '-modulepaths='+InclusionsPath, '-lcbCompilerPath='+lcbconfig.get("LCBCompilerPath", "")];
	...
	private config: ILivecodebuilderConfig | undefined;
	...
	constructor() {
		...
		this.loadConfigP = this.loadLCBConfiguration();
	}
	...
	private async loadLCBConfiguration(): Promise<void> {
		...
		this.config = await getLCBConfig();
	...
	private doValidate(textDocument: vscode.TextDocument): Promise<void> {
		return new Promise<void>(resolve => {
			...
			let processLine = (line: string) => {
				let matches = line.match(LivecodebuilderValidationProvider.MatchExpression);
				if (matches) {
	...
	private async showError(error: any, executable: string): Promise<void> {
		...
		if (error.code === 'ENOENT') {
			if (this.config!.executable) {
				message = localize('wrongExecutable', 'Cannot validate since {0} is not a valid Livecode executable. Use the setting \'livecodebuilder.LCBCompilerPath\' to configure the Livecode executable.', executable);
			} else {
				message = localize('noExecutable', 'Cannot validate since no Livecode executable is set. Use the setting \'livecodebuilder.LCBCompilerPath\' to configure the Livecode executable.');
			}
		} else {
...
interface ILivecodebuilderConfig {
	readonly executable: string | undefined;
	readonly executableIsUserDefined: boolean | undefined;
	readonly trigger: RunTrigger;
}

async function getLCBConfig(): Promise<ILivecodebuilderConfig> {
	const section = vscode.workspace.getConfiguration();
	...
	if (executable && !path.isAbsolute(executable)) {
		...
		executable = await getLivecodebuilderPath();
	}
...
async function getLivecodebuilderPath(): Promise<string | undefined> {
	try {
		return await which('livecodebuilder');
	} catch (e) {
		return undefined;
	}
}
